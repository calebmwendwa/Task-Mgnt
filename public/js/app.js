$(document).ready(function() {
 ...
 if(typeof brands_table !== 'undefined') {
    ...
    if ($('form#bussiness_edit_form').length > 0) {
        ...
        $('#cac').fileinput(fileinput_setting);
        $('#photo_image').fileinput(fileinput_setting);
        $('#prove_of_residence').fileinput(fileinput_setting);
    }
...
$(document).on('click', 'button.load_notifications', function(e) {
    e.preventDefault();
...
function updateProfitLoss(start = null, end = null, location_id = null, selector = null) {
    ...
    $.ajax({
        ...
        success: function(html) {
            ...
            if($('#profit_by_categories:visible').length != 0){
                $('#profit_by_categories_table').DataTable().ajax.reload();
            } else if($('#profit_by_brands:visible').length != 0) {
                $('#profit_by_brands_table').DataTable().ajax.reload();
            } else if($('#profit_by_locations:visible').length != 0) {
                $('#profit_by_locations_table').DataTable().ajax.reload();
            } else if($('#profit_by_invoice:visible').length != 0) {
                $('#profit_by_invoice_table').DataTable().ajax.reload();
            } else if($('#profit_by_date:visible').length != 0) {
                $('#profit_by_date_table').DataTable().ajax.reload();
            } else if($('#profit_by_day:visible').length != 0) {
                $('#profit_by_day_table').DataTable().ajax.reload();
            } else if($('#profit_by_customer:visible').length != 0) {
                $('#profit_by_customer_table').DataTable().ajax.reload();
            } else {
                $('#profit_by_products_table').DataTable().ajax.reload();
            }
        },
    });
...
$(document).on('click', 'button.edit_user_button', function() {
        $('div.user_modal').load($(this).data('href'), function() {
            ...
            $('form#submit_user_button').submit(function(e) {
                e.preventDefault();
                var data = $(this).serialize();
...
$('.sms_setting_add_modal, .sms_setting_edit_modal').on('shown.bs.modal', function(e) {
    $('form#sms_settings_add_form')
        .submit(function(e) {
            e.preventDefault();
        })
        .validate({
            submitHandler: function(form) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.success == true) {
                            $('div.sms_setting_add_modal').modal('hide');
                            $('div.sms_setting_edit_modal').modal('hide');
                            toastr.success(result.msg);
...
$(document).on('submit', 'form#edit_payment_form', function(e){
    e.preventDefault();
    var form = $(this);
    var data = form.serialize();
    $.ajax({
...
$('.report_transaction_modal').on('shown.bs.modal', function(e) {
    $('form#report_transaction')
        .submit(function(e) {
            e.preventDefault();
        })
        .validate({
            submitHandler: function(form) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.success == true) {
                            $('div.report_transaction_modal').modal('hide');
                            toastr.success(result.msg);
                        } else {
                            toastr.error(result.msg);
                        }
...
$('.payout_modal').on('shown.bs.modal', function(e) {
    $('.payout_modal')
        .find('.select2')
        .each(function() {
            __select2($(this));
...
$(document).on('click', '#account_verification', function() {
    ...
    $.ajax({
        method: 'POST',
        url: '/business/payouts/verification',
        dataType: 'json',
        data: {bank_code: $('#bank_code').val(), account_number: $('#account_number').val(), bvn:$('#bvn').val()},
        success: function(result) {

            if (result.success) {
                $('#found').empty().prepend(result.data.accountName);
                $('input#account_name').val(result.data.accountName);
                $('#account_verification').html('Verify')
                $('.set_up_bvn').show()
                $('#not-found').html('')
            } else {
                $('#not-found').empty().prepend(result.msg);
                $('#account_verification').html('Verify')
                $('.set_up_bvn').hide()
            }
        },
...
$(document).on('click', '#complete_verification', function() {
    $(this).html('Verifying...')

    if ($('#code').val() == '') {
        $('#complete_verification').html('Done!')
        return toastr.error("Enter Valide Code");
    }
...
$(document).on('click', '#update_customer_phone_number', function () {

    if ($("#phone_number").val().length != 11) {
        return toastr.error('Invalide Contact');
    }
...
$(document).on('click', '#change_phone_number', function () {
    $(".verify_content").hide()
    $(".edit_phone_content").show()
});

$(document).on('click', '.business_owner_email_verification', function () {
    $.ajax({
        method: 'POST',
        url: '/user/update-business-owner-email',
        dataType: 'json',
        data: {
            email:$("#business_owner_email_adddress").val()
        },
        success: function(result) {
            if(result.success == true) {
            toastr.success(result.msg);
...
$(document).on('click', '.email_verification', function () {
    $.ajax({
        ...
        dataType: 'json',
        data: {
            email:$("#email_adddress").val()
        },
        success: function(result) {
            if(result.success == true) {
                $('div#email_verification_modal').modal('hide');
                $('div#email_verified_modal').modal('show');
            } else {
                toastr.error(result.msg);
            }
        },
...
$(document).on('submit', 'form#customize_invoice_form', function(e) {
    e.preventDefault();
    $.ajax({
        method: 'POST',
        url: $(this).attr('action'),
        ...
        data: new FormData($('#customize_invoice_form')[0]),
        processData: false,
        contentType: false,
        success: function(result) {
            if (result.success == 1) {
                console.log(result.data);
                $('div.customize_modal').modal('hide');
                $(`#invoice_discount_type option[value='${result.data.default_discount_type}']`).prop('selected', true);
                $(`#invoice_tax_rate option[value='${result.data.tax_rate_id}']`).prop('selected', true);
                $('#invoice_sale_note').val(result.data.footer_text);
                $('#customize_invoice_prefix').val(result.data.invoice_prefix);
                if (result.data.enable_commission_agent == 1) {
                    $('.invoice_commission_agent').removeClass('hide');
                } else {
                    $('.invoice_commission_agent').addClass('hide');
                }
                if (result.data.enable_sales_agent == 1) {
                    $('.invoice_sales_agent').removeClass('hide');
                } else {
                    $('.invoice_sales_agent').addClass('hide');
                }
                toastr.success(result.msg);
            } else {
                toastr.error(result.msg);
...
$('.create_note_modal').on('shown.bs.modal', function() {
    $(document).on('submit', 'form#invoice_note_add_form', function(e) {
        e.preventDefault();
        var data = $(this).serialize();
            ...
            success: function(result) {
                if (result.success === true) {
                    ...
                    if (result.note.description) {
                        var delete_url = '/invoice-notes/' + result.note.id;
                        var edit_url = '/invoice-notes/' + result.note.id + '/edit';
                        var surname = result.note.created_user.surname !== null ? result.note.created_user.surname : '';
                        var first_name = result.note.created_user.first_name !== null ? result.note.created_user.first_name : '';
                        var last_name = result.note.created_user.last_name !== null ? result.note.created_user.last_name : '';
                        var html = '<tr>';
                        html += '<td>' + surname + first_name + last_name + '</td>';
                        html += '<td>' + result.note.description + '</td>';
                        html += '<td>' + result.note.date + '</td>';
                        html += '<td>';
                        html += '<div class="invoice-table-options">';
                        html += '<button type="button" data-href=' + edit_url + ' class="edit_invoice_note">';
                        html += "<img src='/img/icons/edit.svg' alt='' />";
                        html += '</button>';
                        html += '<button type="button" data-href=' + delete_url + ' class="delete_invoice_note">';
                        html += "<img src='/img/icons/delete.svg' alt='' />";
                        html += '</button>';
                        html += '</div';
                        html += '</td>';
    ...
    $(document).on('click', 'button.delete_invoice_note', function() {
        swal({
            title: LANG.sure,
            icon: 'warning',
            buttons: true,
    ...
    $(document).on('click', 'button.edit_invoice_note', function() {
        var href = $(this).data('href');
        var data = $(this).serialize();
        var currentRow = $(this).closest("tr");
        currentRow.remove();
        $.ajax({
            ...
            success: function(result) {
                if (result.success === true) {
                    $('#note_description').val(result.note.description);
                    $('#note_id').val(result.note.id);
                    $('.note_btn').html('Update Note');
                } else {
                    toastr.error(result.msg);
                }
            },
...
$('.create_reminder_modal').on('shown.bs.modal', function() {
    $(document).on('submit', 'form#invoice_reminder_add_form', function(e) {
        e.preventDefault();
        var data = $(this).serialize();
            ...
            success: function(result) {
                if (result.success === true) {
                    ...
                    if (result.reminder.description) {
                        var surname = result.reminder.created_user.surname !== null ? result.reminder.created_user.surname : '';
                        var first_name = result.reminder.created_user.first_name !== null ? result.reminder.created_user.first_name : '';
                        var last_name = result.reminder.created_user.last_name !== null ? result.reminder.created_user.last_name : '';
                        var html = '<tr>';
                        html += '<td>' + result.reminder.description + '</td>';
                        html += '<td>' + result.reminder.date + '</td>';
                        html += '<td>' + surname + first_name + last_name + '</td>';
                        if(result.reminder.status == 1){
                            html += '<td><span class="label bg-light-green">Sent</span></td>';
                        }else{
                            html += '<td><span class="label bg-yellow">Not Sent</span></td>';
                        }
                        html += '</tr>';
                        $('#table_data').prepend(html);
...
$(document).on('click', '#mark_as_sent', function(){
    var transaction_id = $('#invoice_transaction_id').val();
    $.ajax({
        method: 'POST',
        url: '/sells/mark-as-sent',
        dataType: 'json',
        data: {
...
$(document).on('click', 'button#delete_invoice', function() {
    swal({
        title: LANG.sure,
        icon: 'warning',
        buttons: true,
